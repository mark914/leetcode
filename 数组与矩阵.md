...
求连续子数组的和，满足条件的连续子数组的最小长度。这种就可以使用滑动窗口方法来解决
...
在排好序的nums中找到第一个大于等于target的数nums[i]，然后将nums[i] 换为target，nums仍然是一个有序的数组
...
n + 1个数中选出n个数，找出没有被选到的那个数

从[0, n]中选,坐标为 [0, n-1]

方法一：使用求和的方法，可能回超出整数的最大范围，溢出。因此需要边加边减。

方法二：排序，然后遍历一遍，或者用二分查找

方法三：使用hashset，将出现的都存储下来。看[0, n]中哪个没出现

方法四：位运算，索引为[0, n), 数为[0, n]中的n-1位，添加以为n，则除了缺失的数外，其它都出现两次，就可以使用~操作

[448找到所有数组中消失的数字](https://leetcode-cn.com/problems/find-all-numbers-disappeared-in-an-array/)

方法一：原地哈希

建立nums[i] 和 索引之间的一一对应关系

如果已经放进map里面，就不需要操作了

最后再看下那些数字没有被放到map里面，就可以确定消失数字了。
...
索引[0, n), 数字[1, n]，其中有一个数字被替换为另一个数字。

建立一个map，遍历数组，看每个元素是否出现过。
