...
百度百科https://baike.baidu.com/item/%E4%BD%8D%E8%BF%90%E7%AE%97/6888804

0s 表示一串 0，1s 表示一串 1。

```java
x ^ 0s = x（和0不同的只有1）      x & 0s = 0 （结果由0决定）     x | 0s = x (结果由1决定)
x ^ 1s = ~x（和1不同的只有0）    x & 1s = x （1会被保留）     x | 1s = 1s （结果由1决定）
x ^ x = 0 （没有不同的）      x & x = x  （1会被保留）     x | x = x （1会被保留）
```
...
- 使用异或将不同的位置转化为1，相同的位置转化为0（z)
- 统计z中1的个数
  - z & 1可以判断最后一位是否为1
  - Integer.bitCount(z)能够确定z中有多少个1
  - z &(z-1)会将z中的1减少一个
...
思路：n个数和n-1个数的和之差就是我们想要的结果。

关键点：

- 这里的n就是nums.length.
- 如果直接使用数学上的加法，会有溢出风险，所以可以边加边减去。 总共n+1个数，和为1-n；而nums中则为0-n-1所以可以边加边减。
...
思路：使用set，巧用add，contains，remove等操作

关键：

set是无序的，不能使用索引获取，但是可以使用toArray方法。
...
public int[] singleNumber(int[] nums) {
    ...
    //first add the remove, all the remain is once.
    for (int num: nums) {
...
所有num异或，得到两个唯一的数a，b的异或z，z中为1则表示该位置上a、b两数不相同。

就可以让该位置上为1的为一组，为0的为1组。从而将a，b分到两个组，然后分组异或，就能够得到结果。
