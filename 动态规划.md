...
思考：使用递归 时间复杂度为$2^n$, 空间复杂度为n，就算是记忆递归，时间复杂度为n，空间复杂度也为n

使用动态规划，时间复杂度为n，空间复杂度为1
...
for loop和while loop一定要区分好。

while loop只需要一个条件，

for loop需要很多条件

在while 和 for loop中定义的变量，不能再外面使用，这个是非常重要的。

关键：

状态转移方程：dp[i] = Math.max(nums[i] + dp[i-2], dp[i-1]);

边界条件: length = 1, i = 0; length = 2, i = 1; 	

因为dp[0] 对应length = 1; dp[1] 对应length = 2；所以dp[n]对应length = n;所以就可以创建一个长度为length的状态array来存储所有状态。

但根据状态转移方程，第i次的结果至于其前两次的结果相关，因此可以使用滚动数组。在每个时刻只需要存储前前两次的结果（即前两个房屋的最高总金额）

```java
public int rob(int[] nums) {
    int length = nums.length;
    //用length来决定抢第length个房子的最高总金额
    if (length == 1) return nums[0];
    int prev1 = nums[0];
    int prev2 = Math.max(nums[0], nums[1]);
    
    for (int i = 2; i < length; i ++) {
        //当i=length时退出循环，返回结果prev2,因为prev2是最新计算出来的结果。 length = 2则不需要计算，length  = 3，则需要向前计算一次。
        int temp = Math.max(nums[i] + prev1, prev2);
        prev2 = temp;
        prev1 = prev2;
    }
    return prev2;
...
思路：将环形转化为两个单排问题，也就是上一个问题。

rob(int[] nums, int first, int end);

- 前两个为 prev1 = nums[first]; prev2 = nums[first + 1];
- 开始循环需要从i = first + 2开始，直到 i = end时最后一次循环。相当于把nums的最后一个位置的结果给求出来。

其实可以理解为求每一个位置的状态，循环的次数也为只要求出所有状态就好了。上一题中i = length - 1时为最后一次循环，是因为最后一个状态位置就是length -1。
...
思路：最小路径和，既然是求和，那就需要一项一项求和，前i项的和可以分解为前i-1项的和加上当前项。所以就可以考虑用动态规划来做。

动态规划的关键在于动态转移方程以及最后要求的状态，该问题存在多个转移方程，最后的状态为grid[m-1]【n-1】
...
思路： 使用动态规划方法很简单，其实就是对每一个位置的状态进行填充，每一个位置只能从上面或者左边到达，因为dp[i]【j】为左边和上边的状态之和。
...
思路：首先想到的是 用sums[i]表示前i个nums的和（包括i），sum[i] = sums[i-1]+nums[i]，相当于前面i-1个的和加上第i个的和。但这样sums[0] = nums[0]+sum[-1]这明显是不太行的。

所以这时就设置了一个dummy位置，sum[0] = 0; 对任意的i都有 sums[i] = sums[i-1] + nums[i-1]，sums[i]表示前i-1个数字的和。所以sums中有效的位置为1~n。 sumRange(i , j) = sums[j+1] - sums[i];
...
问题：将一正整数转化为平方和的形式，并要求平方和的数目尽量少。

思考：

很明显，平方数本身的结果为1

**对正整数i， 最大结果就是i,  假设我们已经知道dp[i]之前所有位置的状态结果，dp[i] 就是所有dp[i- j*j]中最小值加一。 问题就转换为如何求一组数的最小值。**

**这其实就是递归的思想了，因为我们假设了边界条件，并且也有不断简化问题的方法（条件转移方程），最终所有问题都转化为最简单的条件。只不过再动态规划中，我们使用array dp记下来不同状态的结果。从而减少了时间复杂度。**

**也即在递归的基础上 用空间换时间。**

dp[i] = i;

for each j = 1, i-j*j >= 0, j++:
...
目标：求dp[n]代表前n-1个元素对应的所有解码方式

边界条件： 

dp[0] = 1; 这是因为如果是s是两位数，那只要它小于等26就能够成为一组，此时dp[i] += dp[i-2]就需要dp[0]作为初始条件。

dp[1] = s.charAt(0) == '0' ? 0: 1; 

递推公式：

- 新添加的元素能不能独立成为一个 dp[i] += dp[i-1];
  - 只要新添加的元素不为0即可
- 新添加的元素能不能和前一个元素一起独立成为一个 dpp[i] += dp[i-2]
  - 只要新添加的元素和前一个元素组合起来小于等于26即可
  - 注意，此时前一个元素不能为0
...
s[i-1]能否独当一面，s[i-1]和s[i-2]结合起来能否独当一面，如果能够独挡一面，那就和前面的数量一致。
...
dp[n] 表示以Sn结尾的最长递增子序列的长度

边界条件：dp[1] = 1;

递推公式：dp[n] = max(1, dp[i] + 1|dp[i] < dp[n] && i < n)

关键：dp[i]与前面所有的状态都有关，所以需要创建一个dp数组来存储这些所有的状态。
...
-  求一列数的最大值，这个真的非常重要。该题用了两次。思路就是遍历并保存现有数中的最大值。
- 创建数组int[] a = new int[n];
- 每句话后面要加；
...
思路：和上一题的解法一模一样，只不过这里题目是说按照任意顺序，所以我们首先要对pairs进行排序。

Array.sort(pairs, (a, b) -> a[0]-b[0])) //这里的comparator应该是谁小就让谁排在前面。

我们把pairs按照每个数对第一个元素的大小从小到大进行排序。

python中sort的使用方法：

```python
list.sort(reverse = True, key = lambda a:a[0])
```
...
<img src="https://inews.gtimg.com/newsapp_ls/0/14486473817/0.jiketuchuang.png" alt="image.png" title="image.png" />

注意：这里是如何表示两个单词的，纵轴是一个单词（长为ｍ），横轴是一个单词（长为ｎ）。

**用ｐｙｔｈｏｎ初始化的时候：　ｄｐ　＝　［［０］＊ｎ　ｆｏｒ　＿　ｉｎ　ｒａｎｇｅ（ｍ）］前面的是列的大小，后面的ｍ是行的大小。也就是先列后行。**

要使用二维dp进行解决：

为什么

如何理解两个String序列的公共子序列：

$dp[i][j]$表示String a的前i个字符和String b的前j个字符之间的最长公共子序列。

边界条件，如果$dp[i][0] = 0$,  $dp[0][j] = 0$

递推公式：如果a[i-1] == b[j-1]，那么$dp[i][j] = dp[i-1][j-1]+1$ , 如果a[i] != b[j] 那么$dp[i][j] = max(dp[i][j-1], dp[i-1][j])$
...
思路：和276 找一个正整数的最小数目平方和是同一个思路；

- 首先给出状态array dp

  - 因为我们的目标是求出amount对应的状态dp[amount]，所以我们把dp的长度设置为amount+1

- 给出初始值和边界值：

  - 因为后面要求一列数的最小值，所以先给dp赋值为不可能的较大数amount+1。

- 并给出base case dp[0] = 0;

  - 然后根据状态转移方程，自下而上求出dp[amount]。其中涉及到求一组数的最小值。

    for each val in coins:

    if (i - val < 0) continus;

    dp[i] = Math.min(dp[i], dp[i- val] + 1);

- 最后的关键点在于如何判断该数是否能够被组合，主要看dp[amount]是否等于amount+1。如果是则不行，如果不是则可以。
